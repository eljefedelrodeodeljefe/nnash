...
class NCache extends Readable {
  ...
  set (key, value, ttl, cb) {
    ...
    if (this.storage.data[key]) {
      existent = true
      this.stats.vsize -= this._getValLength(this._unwrap(this.storage.data[key], false))
    }
    // set the value
    this.storage.data[key] = this._wrap(value, ttl)
  ...
  del (keys, cb) {
    ...
    for (i = 0, len = keys.length; i < len; i++) {
      ...
      if (this.storage.data[key] != null) {
        ...
        this.stats.keys--
        delCount++
        const oldVal = this.storage.data[key]
  ...
  ttl () {
    ...
    if ((this.storage.data[key] != null) && this._check(key, this.storage.data[key])) {
      if (ttl > 0) {
        this.storage.data[key] = this._wrap(this.storage.data[key].v, ttl, false)
      } else {
        this.del(key)
      }
      if (cb != null) {
  ...
  getTTL (key, cb) {
    ...
    if ((this.storage.data[key] != null) && this._check(key, this.storage.data[key])) {
      const _ttl = this.storage.data[key].t
      if (cb != null) {
        cb(null, _ttl)
      }
      return _ttl
